...
# ================================================================
# Generate trade signals from last N hours of twitter_threads
...
# DB connection from env (we'll set these in Actions secrets/vars)
...
if (PG_HOST == "" || PG_USER == "" || PG_PWD == "") {
  stop("Database env vars missing: SUPABASE_HOST/USER/PWD (we'll add them in YAML).")
}
if (Sys.getenv("OPENAI_API_KEY") == "") {
  stop("OPENAI_API_KEY env var is missing (we'll add it in YAML).")
}

# -------------------------- DB: read last N hours ----------------
...
if (nrow(collapsed_tbl) == 0) {
  message(sprintf("No threads in the last %d hours. Nothing to do.", TIME_WINDOW_HOURS))
  quit(status = 0)
}

# Normalize timezones: keep ET as separate column
  ...
  for (k in seq_len(retries)) {
    ...
    if (inherits(resp, "error")) {
      ...
      if (!is.null(parsed$error)) {
        return(paste0("[OPENAI-ERROR] ", parsed$error$message))
      }
      if (length(parsed$choices) > 0) {
        return(str_trim(parsed$choices[[1]]$message$content))
      }
    }
    Sys.sleep(2^k)
  }
...
4) Specificity rubric:
   - 0.0–0.2: vague opinion (e.g., \"Tesla is the future\") no timeframe/catalyst
...
message(sprintf("Processing %d threads from the last %d hours…", nrow(df), TIME_WINDOW_HOURS))
    ...
    gpt_table = map2(gpt_raw, conversation_id, function(txt, cid) {
      ...
      if (length(missing)) {
        for (nm in missing) {
          ...
          out[[nm]] <- rep(NA, nrow(out)) |> as(class(tmpl)[1])
        }
      }
...
.fix_one <- function(sym) {
  ...
  crypto_keep <- c(
    "BTC","ETH","ADA","DOGE","DOT","AVAX","LINK","MANA","1INCH","AAVE","APE",
    "ARB","ALGO","MASK","METIS","MILK","MUTE","MYRO","NEXO","NFT","NFTX",
    "OKB","OCEAN","OXT","PEPE","PYR","PYUSD","RNDR","RSR","RVN","SLERF",
    "SNEK","LQTY","FXS","FRAX","GAS","GMX","KAS","ICP","IMX","ILV","JASMY",
    "KDA","KEEP","DOCK","MELD","MOBILE","DYM","DOLA","SAI","LRC","LUSD",
    "RENBTC","REQ","REP","RLC","RON","AUDIO","BAT","CRO","CVC","DCR","DERO",
    "DIONE","GODS","HNT","ICX","IOTX","LPT","LOOKS","LOOM","LADYS","MKR",
    "MBL","NKN","NOIA","ONT","ORAI","PAAL","PAXG","POLY","PSYOP","QUACK",
    "RAAS","RETH","REVV","SFP","SHIB","SKL","SPELL","SSV","STARL","STEEM",
    "STRD","STRK","SUSHI","SWETH","SYS","TAO","TAROT","TFUEL","THETA","TIA",
    "TLM","TLOS","TORN","TOSHI","UMA","USDT","VEE","VELO","VRSC","WAXP",
    "WBNB","WOO","XAUT","XCH","XCN","XDC","XDEFI","XEC","XEM","XLM","XMR",
    "XPR","XRD","XRP","XVG","XYO","ZANO","ZEC","ZEN"
  )
...
if (nrow(signals_from_tweets) == 0) {
  message("No valid signals produced from GPT for the last window. Exiting.")
  quit(status = 0)
}
